...
def get_pt_operator_registry_dict(
    ...
    lib_deps = [
        ...
        third_party("glog"),
        C10,
    ] + (["//xplat/caffe2/fb/custom_ops/unet_metal_prepack:unet_metal_prepack_header"] if NOT_OSS else [])

    if train:
        lib_deps.append(ROOT + ":torch_mobile_train")

    if enable_flatbuffer:
        lib_deps.append(ROOT + ":torch_flatbuffer_all")

    kwargs.pop("exported_headers", [])
    return dict(
        name = name,
        srcs = kwargs.pop("srcs", []) + code_gen_srcs,
        linker_flags = [
            "-Wl,--no-as-needed",
        ],
        ...
        exported_preprocessor_flags = kwargs.pop("exported_preprocessor_flags", []) + (["-DTEMPLATE_SELECTIVE_BUILD"] if template_select else []),
